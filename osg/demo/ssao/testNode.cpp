...
TestNode::TestNode()
{
	...
	auto grp = osgDB::readNodeFile(path.string() + "/untitled.obj");
	...
	_quad = osg::createTexturedQuadGeometry(osg::Vec3(-1, -1, 0), { 2, 0, 0 }, { 0, 2, 0 });
	{
		...
		ss->setAttribute(pg);
		ss->setTextureAttribute(0, _posTex);
		ss->addUniform(new osg::Uniform("tex0", 0));
		ss->setTextureAttribute(1, _normalTex);
		...
		image->setImage(4, 4, 1, GL_RGB, GL_RGB, GL_FLOAT, (unsigned char*)ssaoNoise.data(), osg::Image::NO_DELETE);
		noiseTex->setImage(image);

		ss->setTextureAttribute(2, noiseTex);
...

	{
		...
		ss->setAttribute(pg);
		ss->setTextureAttribute(0, _ssaoTex);
		ss->addUniform(new osg::Uniform("tex0", 0));
	}
